...
The two endogenous designs agreed in the table above. Do they agree in general? We compared
them by **replication** — not by shared seeds. (Common random numbers align the two
*strategies* within one model, but they cannot align two *different* models: A and B inject
different confirm-handling events into the loop, so the reactive resample consumes the
disease draws at different cadences. Pairing A against B on a shared seed measures that
cadence artifact, not the models. So we run each model over many independent cohorts and
compare the means.)

::: {.callout-note}
## A = B on the bottom line, with a caveat (30 independent replications)
Over 30 independent cohorts (`validation/validate-AB-replication.R`), A and B **agree on the
cost axis**: the field test is cheaper by ≈ \$47/person under both, well within Monte-Carlo
error. On the **health axis** they sit a hair apart and on opposite sides of zero: Approach
B (the exact queue) puts the incremental QALY at about **−0.04** (southwest), while Approach
A (the analytic wait) puts it at about **0.00** — right on the SE/SW boundary. The gap is
small and not statistically sharp at 30 reps, but it is *directional*: A's M/M/c wait is a
touch optimistic under heavy saturation, so it under-states the health loss exactly where
the decision is closest. The convenient model and the faithful model can land on opposite
sides of the line precisely where it matters most.
:::
